      ...
      {summary.extraction_warnings.length > 0 && (
        <div className="rounded-xl border border-amber-400/20 bg-amber-400/5 p-4">
          <div className="flex items-center gap-2 text-sm font-semibold text-amber-300">
            <AlertTriangle className="h-4 w-4" /> Extraction notes
          </div>
          <ul className="mt-2 space-y-1 text-xs text-slate-400">
            {summary.extraction_warnings.map((warning) => <li key={warning}>• {warning}</li>)}
          </ul>
        </div>
      )}
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        <div className="rounded-xl border border-hairline bg-surface-2/20 p-4">
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            {summary.detected_is_numbers.length ? summary.detected_is_numbers.map((item) => (
              <span key={item} className="rounded-md border border-accent/20 bg-accent/5 px-2 py-1 font-mono text-[11px] text-accent">{item}</span>
        ...
        <div className="rounded-xl border border-hairline bg-surface-2/20 p-4">
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            {summary.detected_sections.length ? summary.detected_sections.map((item) => (
              <span key={item} className="rounded-md border border-hairline bg-surface px-2 py-1 text-[11px] text-slate-300">{item}</span>
            )) : <span className="text-xs text-slate-600">No standard section headings detected</span>}
          </div>
        </div>
        <div className="rounded-xl border border-hairline bg-surface-2/20 p-4">
          <h3 className="text-xs font-semibold uppercase tracking-wide text-slate-400">Referenced standards</h3>
          <div className="mt-3 flex flex-wrap gap-2">
            {summary.detected_references.length ? summary.detected_references.map((item) => (
              <span key={item} className="rounded-md border border-hairline bg-surface px-2 py-1 font-mono text-[11px] text-slate-300">{item}</span>
            )) : <span className="text-xs text-slate-600">No IS references detected</span>}
          </div>
        </div>
      </div>
          ...
          <div className="border-t border-hairline px-4 pb-4">
            <div className="divide-y divide-hairline">
              {summary.pages.map((page) => (
                <div key={page.page} className="grid gap-2 py-3 md:grid-cols-[80px_100px_1fr]">
                  <span className="text-xs font-semibold text-slate-400">Page {page.page}</span>
                  <span className="font-mono text-[11px] text-slate-600">{page.characters.toLocaleString()} chars</span>
                  <span className="text-xs leading-5 text-slate-500">{page.preview}</span>
                </div>
              ))}
            </div>
          </div>
  ...
  return (
    <div data-testid="pdf-analysis-page" className="space-y-6">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          <p className="text-xs font-semibold uppercase tracking-[0.18em] text-accent">BIS Intelligence</p>
          <h1 className="mt-2 font-display text-2xl font-bold text-slate-100">PDF Analysis</h1>
          <p className="mt-1 max-w-2xl text-sm text-slate-500">Upload a specification or technical PDF to extract its structure, identify BIS references, and find semantically relevant standards.</p>
        </div>
      </div>
      ...
      <div className="grid gap-6 xl:grid-cols-[420px_1fr]">
        <div className="space-y-5">
          <div className="panel space-y-4 p-5">
            <div>
              <h2 className="font-display text-sm font-semibold text-slate-200">Source document</h2>
              <p className="mt-1 text-xs text-slate-600">Text is extracted page-by-page before semantic matching.</p>
            </div>
            <PdfUploadZone file={file} onFileChange={setFile} onAnalyze={runAnalysis} analyzing={loading} />
        ...
        <div className="min-w-0 space-y-5">
          {loading ? (
            ...
            <>
              ...
              {results.length > 0 && <SemanticMatchChart items={results} />}
              <div className="panel p-5">
                <div className="flex items-center justify-between gap-3">
                  <div>
                    <h2 className="font-display text-lg font-semibold text-slate-100">Recommended standards</h2>
                    <p className="mt-1 text-xs text-slate-500">Matches generated from the extracted PDF content, detected references, and document structure.</p>
                  </div>
                  <span className="rounded-full border border-accent/20 bg-accent/5 px-2.5 py-1 font-mono text-xs text-accent">{results.length} matches</span>
                </div>
                <div className="mt-4"><ResultsList items={results} selectedIds={new Set()} onToggleCompare={() => undefined} /></div>
              </div>
            ...
            <div className="panel flex min-h-[420px] flex-col items-center justify-center p-8 text-center">
              ...
              <h2 className="mt-4 font-display text-xl font-semibold text-slate-200">Ready for document analysis</h2>
              <p className="mt-2 max-w-md text-sm leading-6 text-slate-500">The analysis will inspect the PDF text layer, page coverage, BIS numbers, standard sections, references, and semantic similarity.</p>
            </div>
          )}
        </div>
      </div>
